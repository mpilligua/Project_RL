...
class CustomEvalCallback(WandbCallback):
    ...
    def evaluate_policy(self, calls, policy='train'):
        ...
        while not done and t < 10000:
            ...
            current_state, ball_position, human_position, other_player_position = current_state['masked_observation'], current_state['ball_position'], current_state['human_position'], current_state['other_player_position']
            ball_position = ball_position[0]
            human_position = human_position[0]
            other_player_position = other_player_position[0]
            ...
            human_position = human_position[0]
            other_player_position = other_player_position[0]
            img_input_model.putpixel((int(ball_position[1] * self.image_shape[1]), int(ball_position[0] * self.image_shape[0])), (255, 0, 0))
            img_input_model.putpixel((int(human_position[1] * self.image_shape[1]), int(human_position[0] * self.image_shape[0])), (0, 255, 0))
            img_input_model.putpixel((int(other_player_position[1] * self.image_shape[1]), int(other_player_position[0] * self.image_shape[0])), (0, 0, 255))
...
class CustomCNN(BaseFeaturesExtractor):
    def __init__(self, observation_space: gym.spaces.Box, features_dim: int = 512, env=None):
        super(CustomCNN, self).__init__(observation_space, features_dim)
        # We have H, W, C
        n_input_channels = 4

        self.dropout = nn.Dropout(0.5)
        self.mymlp = torch.nn.Sequential(torch.nn.Linear(7,7, bias=True), torch.nn.ReLU())
    ...
    def forward(self, observations: torch.Tensor) -> torch.Tensor:
        ...
        human_player = observations['human_position'][:,0]
        other_player = observations['other_player_position'][:,0]
        server = observations['server'][:,0]

        ball_position = ball_position.view(-1, 2)
        human_player = human_player.view(-1, 2)
        other_player = other_player.view(-1, 2)
        server = server.view(-1, 1)
        
        return self.mymlp(torch.cat((ball_position, human_player, other_player, server), dim=1))
        #server = server.view(-1, 1)
        #x = self.cnn(image)

        return torch.cat((ball_position, human_player, other_player), dim=1)
...
class TennisWrapper2(gym.Wrapper):
    ...
    def reset(self, **kwargs):
        ...
        obs_image = self.run_reset(obs)
        return obs_image, {}

    def action_mask(self):
        am = np.array([True]*self.action_space.n)
        if (self.server == 0 and self.run_length < (5 if self.current_score == [0,0] else 8)):
            am[[0] + list(range(2,6))] = False
        return am
...
if __name__ == "__main__":
    ...
    name_run = "Tennis_PPOHumanDetectionProba_2"
    print("Overwriting the run name")
    run = wandb.init(project="Freeway", entity="pilligua2", name=name_run, sync_tensorboard=True, group="Tennis", mode="online")
    ...
    def env_make(do_eval=False):
        ...
        Tennisenv = TennisWrapper2(env)
        env = GetBallPositionAndHuman(Tennisenv)
        env = FrameStackObservation(env, stack_size=4)
        env = MapActions(env)
        env = Monitor(env)
        env.return_masks = Tennisenv.action_mask
        return env
    ...
    # Callbacks
    eval_callback = CustomEvalCallback(eval_env, export_path = results_dir, eval_freq=1000, verbose=2)
...
#
    ...
    policy_kwargs = {
        'net_arch': [dict(pi=[6, 6, 6], vf=[6, 6, 6])],
