...
class RainbowDQN_Agent:
    ...
    def take_step(self, epsilon=0.0, mode='explore', device="cpu"):
        ...
        def get_action(state):
            if isinstance(self.dnnetwork, Noisy_DQN):
    ...
    def train(self, gamma=0.99, 
        ...
        while training:
            ...
            if self.frame_number % 10000 == 0:
                ...
                self.dnnetwork.eval() #For the noisy layers, this is important because it will turn off the noise
                create_and_save_gif(self.dnnetwork, 
                                    device=device, 
                                    save_gif=f"/fhome/pmlai10/Project_RL/Tennis/Tennis_{idx}/frame_" + str(self.frame_number) + ".gif")
                self.dnnetwork.train()
    ...
    def calculate_loss(self, batch, weights, idxs, device="cpu"):
        ...
        self.target_network.reset_noise()  # Sample new target net noise
...
import os
idx = os.listdir("/fhome/pmlai10/Project_RL/Tennis").__len__()
wandb.init(project="Tennis", name=f"Tennis_{idx}")
os.makedirs(f"/fhome/pmlai10/Project_RL/Tennis/Tennis_{idx}", exist_ok=True)

print("Saving to: ", f"/fhome/pmlai10/Project_RL/Tennis/Tennis_{idx}")
