...
from utils import get_schedulers
import seaborn
...
class GetBallPosition(gym.ObservationWrapper):
    ...
    #    Apply the mask to the observation
        ...
        return {
            "masked_observation": obs.astype(np.float32) / 255.0,  # Normalize if needed
...
def make_env(env_name, skip=4, stack_size=1, reshape_size=(84, 84), render_mode=None, eval=False):
    ...
    log("GetBallPosition      : {}".format(env.observation_space))
...
#         img_input_model = Image.fromarray(current_state)
...
#         current_state, reward, added_reward, terminated, truncated, _ = env.step(action)
...
#         if added_reward != 0:
#             added_reward_text = f"Add: {added_reward:.2f}"
#             text_bbox = draw.textbbox((0, 0), added_reward_text, font=font)
#             text_width, text_height = text_bbox[2] - text_bbox[0], text_bbox[3] - text_bbox[1]
#             draw.text((img_pil.width - text_width - 10, 30), added_reward_text, fill="white", font=font)
...
class RainbowDQN_Agent:
    ...
    def __init__(self, 
        ...
        if self.dqn_extensions['use_noisy_dqn']:
            ...
            dnnetwork = DQN2(input_shape = self.train_env.observation_space[1].shape, 
    ...
    def visualize_train(self, policy='train'):
        ...
        while not done and t < 100:
            ...
            current_state, reward, added_reward, terminated, truncated, _ = self.eval_env.step(action)
