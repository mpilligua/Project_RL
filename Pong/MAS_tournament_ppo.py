...
name_run = "MAS_PongPPOTurnBased_multiple_envs7"
...
num_envs = 3
print("Creating environment with {} parallel games".format(num_envs))
...
print("Functions from the environment: ", dir(env))
agents = env.possible_agents
print("Agents in the environment: ", agents)
agent_envs = {agent: SingleAgentEnv(env, agent) for agent in agents}
...
policies = {
    agent: PPO("CnnPolicy", agent_envs[agent], verbose=1, policy_kwargs=policy_kwargs, device=device, n_epochs=5, 
...
# policies[agents[0]].load("/ghome/mpilligua/RL/Project_RL/Pong/runs/PongPPO5/ppo_pong.zip")
# print(policies[agents[0]].policy)
# exit(0)

# Training loop for turn-based environment
total_timesteps = 5000000

episode_rewards = [[] for _ in agents]
log_interval = 1

for agent in agents:
    policies[agent].prev_to_loop(total_timesteps)
    policies[agent].agent_name = agent
    policies[agent].policy.to(device)
    policies[agent].rollout_buffer.n_envs = num_envs
    policies[agent].rollout_buffer.buffer_size = batch_size

logger = AggregatingLogger(policies[agents[0]].logger)
policies[agents[0]].set_logger(logger)
policies[agents[1]].set_logger(logger)
...
while num_timesteps < total_timesteps:
    n_steps = 0
    for agent in agents:
        policies[agent].setup_loop_over_timesteps()
        
    env.reset()  # type: ignore[assignment]
    # print(len(env.last()))
    new_obs, _, _, _, _ = env.last()
    
    for agent in agents:
        policies[agent]._last_episode_starts = np.ones((env.num_envs,), dtype=bool)
    
    last_new_obs = {}
    last_dones = {}
    progress_bar = tqdm(total=(batch_size)*2, position=0, desc="Filling buffer")
    for agent in env.agent_iter(2**63):
        actions, values, log_probs = policies[agent].get_action(new_obs, n_steps)
        # print(f"Actions: {actions}")
        env.step(actions)
        new_obs, rewards, terminated, truncated, infos = env.last() 
        dones = np.array([term or trunc for term, trunc in zip(terminated, truncated)])

        num_timesteps += env.num_envs  
        policies[agent].update_buffer(new_obs, rewards*-1, dones, infos, actions, values, log_probs, num_timesteps // 2)
        n_steps += 1
        progress_bar.update(1)
        
        if n_steps >= (batch_size)*2 - 1:
            last_dones[agent] = dones
            last_new_obs[agent] = new_obs
        
        if n_steps >= (batch_size*2):
            break
        
    for agent in agents:
        # print(policies)
        policies[agent].predict_last_seen(last_new_obs[agent], last_dones[agent])

    iteration += 1
    
    # policies[agents[0]].info["episode"] = env.info["episode"]
    for agent in agents:
        policies[agent].train_and_log(iteration, log_interval, total_timesteps)
    
    wandb.log(logger.get_logs())
    policies[agents[0]].logger.dump(step=num_timesteps)
    
    if iteration % 3 == 0:
        print(f"Visualizing at iteration {iteration}")
        visualize_train(eval_env, policies, episode=iteration, save_path=f"{save_path}/episode_{iteration}.gif")

    if iteration % 15 == 0 and iteration > 0:
        print(f"Saving models at iteration {iteration}")
        for agent in agents:
            # policies[agent].policy.save(f"{save_path}/ppo_{agent}")
            torch.save(policies[agent].policy.state_dict(), f"{save_path}/ppo_{agent}_state_dict")

visualize_train(eval_env, policies, episode=iteration, save_path=f"{save_path}/episode_last.gif")
...
for agent in agents:
    policies[agent].callback.on_training_end()
